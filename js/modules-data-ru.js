...
var MODULES_RU = {
  ...
  m12: {
    ...
    tasks: {
      ...
      'm12-hw3': {
        title: 'Изучить настоящие заголовки ответа',
        goal: 'Выполните GET <code>productsList</code>, затем напечатайте <code>r.headers["Content-Type"]</code>. Убедитесь, что где-то в значении упоминается <b>json</b>.',
        hint: '<code>r.headers</code> ведёт себя как словарь метаданных О ответе, отдельно от <code>r.json()</code>, который является настоящим содержимым тела — <code>Content-Type</code> это сервер, сообщающий, какой формат ожидать, ещё до того как вы начали разбор.'
      }
    }
  },
  m13: {
  ...
  m14: {
    ...
    tasks: {
      ...
      'm14-t4': {
        ...
        goal: 'Перейдите на <code>https://automationexercise.com/login</code>. Найдите поле email через <code>driver.find_element(By.CSS_SELECTOR, \'input[data-qa="login-email"]\')</code> и напечатайте <code>.get_attribute("name")</code>. Убедитесь, что видите <b>email</b>.',
        hint: 'Атрибуты <code>data-qa="..."</code> существуют на этом сайте специально, чтобы дать автоматизации стабильный способ найти элементы, независимо от визуальных классов стилей, которые могут измениться — реальный паттерн, стоящий узнавания при встрече на других сайтах.'
      },
      'm14-t5': {
        title: 'Ввести в поле и прочитать обратно',
        goal: 'На странице входа найдите поле email и вызовите на нём <code>.send_keys("test@example.com")</code>. Затем напечатайте <code>.get_attribute("value")</code> на ТОМ ЖЕ элементе. Убедитесь, что показывается ровно то, что вы ввели.',
        hint: '<code>send_keys</code> по умолчанию не перезаписывает — она печатает поверх того, что уже там есть. На пустом поле вроде этого это не важно, но стоит помнить об этом для поля, в котором уже может быть текст.'
      },
      'm14-t6': {
      ...
      'm14-t8': {
        ...
        goal: 'На главной странице дождитесь, что ссылка «Cart» кликабельна, через <code>WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.LINK_TEXT, "Cart")))</code>, кликните её, затем напечатайте <code>driver.current_url</code>. Убедитесь, что URL теперь содержит <b>view_cart</b>.',
        hint: '<code>EC.element_to_be_clickable</code> проверяет и что элемент существует, И что ничего сейчас его не перекрывает — более строгое, более полезное ожидание, чем просто проверка присутствия, когда следующий шаг — клик.'
      },
      'm14-t9': {
      ...
      'm14-t10': {
        ...
        goal: 'Напишите <code>class LoginPage:</code> с <code>__init__(self, driver)</code>, сохраняющим driver, методом <code>open(self)</code>, переходящим на URL входа, и методом <code>enter_email(self, email)</code>, находящим поле email и отправляющим в него данный текст. Создайте экземпляр, вызовите <code>.open()</code>, затем <code>.enter_email("alex@example.com")</code>, затем прочитайте значение поля напрямую через <code>page.driver.find_element(...)</code>, чтобы подтвердить, что сработало.',
        hint: 'Это синтаксис классов из модуля 7, оборачивающий вызовы Selenium из модуля 14 — ровно та форма, что принимает настоящий Page Object: класс знает, КАК взаимодействовать с одной страницей, тест просто вызывает его методы по имени.'
      },
      'm14-hw1': {
  ...
  m16: {
    ...
    tasks: {
      ...
      'm16-t6': {
        ...
        goal: 'Создайте файл <code>requirements.txt</code>, перечисляющий <code>pytest</code>, <code>playwright</code> и <code>requests</code>, по одному на строку. В НОВОМ пустом виртуальном окружении (или хотя бы концептуально) убедитесь, что <code>pip install -r requirements.txt</code> установил бы всё, что нужно этому проекту, одной командой.',
        hint: 'Выполните <code>pip freeze</code>, чтобы увидеть точные установленные версии на вашей машине — фиксация точных версий (например, <code>pytest==9.1.1</code>) делает проект воспроизводимым; оставление их без фиксации всегда ставит последнюю версию.'
      },
      'm16-t7': {
