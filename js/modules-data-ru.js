...
var MODULES_RU = {
  ...
  m7: {
    ...
    theory: [
      ...
      'Класс может строиться на другом через <code>class Child(Parent):</code> — потомок автоматически получает все атрибуты и методы родителя без копипаста; вы добавляете только то, что для потомка ново или отличается. Именно так настоящий проект на Page Object не повторяет один и тот же <code>__init__(self, base_url)</code> и настройку браузера в каждом классе страницы: один общий <code>BasePage</code> хранит это один раз, а каждая конкретная страница (<code>LoginPage</code>, <code>ProductsPage</code>, ...) наследует его и добавляет только свои методы.'
    ],
    tasks: {
      'm7-t1': {
      ...
      'm7-hw4': {
        title: 'Страница, которая наследует другую страницу',
        goal: 'Спроектируйте класс <code>BasePage</code>, который хранит базовый URL и может вернуть его через метод <code>open()</code> (форма из теории этого модуля). Затем создайте класс <code>ProductsPage</code>, который наследуется от <code>BasePage</code> и добавляет метод <code>search_url(self, term)</code>, возвращающий базовый URL, затем <code>/products?search=</code>, затем переданный термин. Создайте <code>ProductsPage</code> для <code>https://automationexercise.com</code>, напечатайте результат его <code>open()</code>, затем напечатайте <code>search_url("dress")</code>.',
        hint: '<code>class ProductsPage(BasePage):</code> означает, что у <code>ProductsPage</code> уже есть всё, что определено в <code>BasePage</code>, включая его <code>__init__</code> и <code>open()</code> — переписывать ни то, ни другое не нужно. Добавьте только один новый метод: <code>search_url</code>.'
      }
    }
  },
  m8: {
    ...
    theory: [
      ...
      '<code>random.randint(a, b)</code> даёт случайное целое число между <code>a</code> и <code>b</code> (оба включительно); <code>random.choice(a_list)</code> выбирает один случайный элемент из списка. Генерация тестовых данных — случайные ID, случайные валидные входные значения — одно из самых частых реальных применений этого модуля в автоматизации. <code>random.seed(n)</code> — это «кнопка повтора»: она не выбирает значение, она выбирает, по КАКОЙ фиксированной цепочке псевдослучайных чисел пойдут все следующие вызовы. Один и тот же seed всегда даёт одну и ту же цепочку (seed 1 для <code>randint(1, 6)</code> всегда даёт 2, потом 5), другой seed — другую цепочку, а логической связи между числом seed и результатом нет. Настоящий тест берёт свежий seed, печатает его в лог, и когда прогон упал на неудачных данных, вы подставляете записанное число обратно в <code>random.seed()</code> и получаете ровно те же данные заново. Фиксированные seed в примерах и задачах этого модуля нужны только для того, чтобы напечатанный ответ можно было проверить.',
      '<code>datetime.now()</code> (из модуля <code>datetime</code>) даёт текущие дату и время — при каждом запуске они разные, так что на следующий день ничего править не нужно. Код автоматизации постоянно использует это для вещей вроде временной метки строки лога или уникального имени отчёта о прогоне тестов. <code>datetime(2026, 9, 16, 14, 30)</code>, наоборот, собирает один КОНКРЕТНЫЙ момент из года, месяца, дня, часа и минуты — это удобно, когда нужна известная фиксированная дата, и именно поэтому простой пример и задачи этого модуля используют его (фиксированная дата даёт проверяемый ответ). <code>.strftime(шаблон)</code> превращает любой момент в текст: <code>%Y</code> — год, <code>%m</code> — месяц, <code>%d</code> — день, <code>%H</code> — часы, <code>%M</code> — минуты.',
    ...
    tasks: {
      ...
      'm8-t5': {
        title: 'Определённая временная метка'
      },
      'm8-t6': {
  ...
  m11: {
    ...
    theory: [
      ...
      'Заголовок этого модуля говорит «моки И стабы» — вот в чём реальная разница, поскольку <code>unittest.mock</code> в Python намеренно её стирает. <b>Стаб</b> — самая простая подделка: он просто возвращает заготовленные данные при вызове, ничего больше (<code>Mock(return_value=...)</code>, используемый чисто ради возвращаемого значения, по сути и есть стаб). <b>Мок</b> делает то же самое И запоминает, как его вызывали, так что тест позже может проверить само взаимодействие (<code>assert_called_with(...)</code>) — каждый <code>Mock()</code> в этом модуле технически способен быть и тем, и другим; что именно вы используете — вопрос того, что реально проверяет ваш тест, а не того, какой класс вы импортировали.',
      'Единственное правило, отделяющее здоровый мок от анти-паттерна: мокайте зависимость НА ГРАНИЦЕ того, что вы тестируете (платёжный шлюз, почтовый сервис, текущее время), но никогда — саму вещь, ради проверки которой существует тест. <code>test_search_product</code> из модуля 12 всё ещё должен делать настоящий вызов <code>requests.get(...)</code> — замокать его означало бы получить тест, который всегда проходит и ничего не проверяет. Тест вокруг <code>checkout()</code>, мокающий <code>payment_gateway</code>, — правильный, потому что тест про логику оформления заказа, а не про то, работает ли настоящий платёжный шлюз.'
    ],
    tasks: {
      'm11-t1': {
  ...
  m15: {
    ...
    theory: [
      ...
      'Так что же выбрать для нового проекта — теперь, когда изучены оба? Большинство новых проектов браузерной автоматизации сегодня начинают с Playwright: автоматическое ожидание убирает целый класс нестабильных тестов, а собственные проверки построены именно для веба, а не пристроены к общему <code>assert</code>. Selenium по-прежнему заслуживает места на уже существующем проекте, построенном на нём, когда конкретный инструмент в вашем стеке интегрируется только с протоколом <code>WebDriver</code> у Selenium, или когда команде нужен браузер, который Playwright не поддерживает. Знать оба, как только что научил этот курс, важнее, чем выбрать сторону — но если ничто конкретное не тянет вас в другую сторону, начинайте с Playwright.'
    ],
    tasks: {
      'm15-t1': {
  ...
  m16: {
    ...
    theory: [
      ...
      'Две честные оговорки о том, где этот фреймворк сознательно минимален, а не «закончен»: во-первых, каждая задача этого модуля собирает фикстуру Playwright из модуля 15 вручную (<code>with sync_playwright()...</code>), чтобы сделать механику setup/teardown видимой — настоящий проект скорее поставил бы плагин <code>pytest-playwright</code>, который сразу даёт готовую фикстуру <code>page</code> без единой строки bootstrap-кода, плюс CLI-флаги для видео/трейсинга при падении; стоит знать, что он существует, раз ручная версия здесь уже понятна. Во-вторых, этот фреймворк печатает прямо в консоль так же, как это делал каждый модуль до него — настоящий проект обычно переходит на модуль <code>logging</code> из стандартной библиотеки, как только вырастает за пределы горстки тестов, потому что у logging есть уровень серьёзности и метка времени, и его можно перенаправить в файл или сервис логов CI, тогда как <code>print()</code> нельзя выборочно отключить или отфильтровать по важности.'
    ],
    tasks: {
      'm16-t1': {
