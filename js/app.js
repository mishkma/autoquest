...
(function(){
  ...
  function renderPath(){
    ...
    MODULES.forEach((m, idx) => {
      ...
      // A "BOSS" tag + the boss task's own real title, to the LEFT of the
      // icon (10 Sept 2026, user feedback: the spiky silhouette alone read
      // as unlabeled decoration; a first pass stacked just "BOSS" above the
      // icon, which the user then explicitly rejected — they want the name
      // too, positioned to the icon's left, not above it). No invented
      // monster name — this course has never had those, taskField(...,
      // 'title') is the real, already-existing boss task title (e.g. "Test
      // run report"), same string used in renderBossHeader/showVictory.
      ...
      const bossPreview = bossTask
        ? `<div class="node-boss-wrap${state.completed[bossTask.id] ? ' defeated' : ''}">
             <div class="node-boss-text">
               <div class="node-boss-label">${tr('bossLabel')}</div>
               <div class="node-boss-name">${escapeHtml(taskField(bossTask, m, 'title'))}</div>
             </div>
